...
const getStakedBalance = async (address) =>
  StakingToken.methods.balanceOf(address).call({ from: defaultAddress })
const getStakedRewards = async (address) =>
...
const getAccountSkillReward = async (address) =>
  CryptoBlades.methods.getTokenRewards().call({ from: address })
const getCharacterExp = async (address, charId) =>
  CryptoBlades.methods.getXpRewards(`${charId}`).call({ from: address })
const getCharacterStamina = async (charId) =>
  Characters.methods
    .getStaminaPoints(`${charId}`)
    .call({ from: defaultAddress })
const getCharacterData = async (address, charId) =>
  Characters.methods.get(`${charId}`).call({ from: address })
